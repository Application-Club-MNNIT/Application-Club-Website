 ...
 */
const fetchMultipleCodeforcesRatings = async (
  usernames: string[]
): Promise<FormattedCodeforcesUser[]> => {
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      // console.log(data);
      const ratingData: any[] = await User.find({
        "codeforces.username": { $in: usernames },
      })
        .select("+codeforces")
        .lean();
      ...
      const usersData = data.result.map((user) => {
        const uservalue: any = ratingData.filter((userdata) => {
          return userdata.codeforces.username == user.handle;
        });
      ...
      console.error(
        `Error fetching Codeforces data for batch for `,
        handlesParam,
        error.message
      );
...
const getCodeforcesLeaderboard = async (
...
const setCodeforcesLeaderboardCache = (
  leaderboardData: FormattedCodeforcesUser[]
): void => {
