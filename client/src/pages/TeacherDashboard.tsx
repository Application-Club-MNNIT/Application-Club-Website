...
const TeacherPage: React.FC = () => {
  ...
  const getAllTeachers = async () => {
    try {
      const response = await axios.get("http://localhost:8000/teacher/getAll");
  ...
  const addTeacher = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/teacher/add", {
        name,
        email
      });
      ...
      console.error("Add teacher error:", err.response?.data?.message || err.message);
  ...
  const removeTeacher = async (id: string) => {
    try {
      const response = await axios.delete("http://localhost:8000/teacher/delete", {
        data: { teacherId: id }
      });
      ...
      alert(err.response?.data?.message || "Could not delete teacher.");
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 md:px-10 lg:px-16">
      ...
      <AnimatedWrapper>
        <div className="bg-neutral-900 text-white p-8 sm:p-10 min-w-[95vw] sm:min-w-[600px] md:min-w-[700px] lg:min-w-[800px] rounded-lg shadow-lg flex flex-col md:flex-row gap-6">
          {/* Input Section */}
