...
function App() {
  ...
  const [year, setYear] = useState<string | null>("2027");
    ...
    const fetchLeaderboardData = async () => {
      try {
        ...
        if (activeTab === "github") {
          ...
          const response = await backend.get<GithubLeaderboardResponse>(
            "/api/leaderboard/github"
          );
          ...
          const response = await backend.get<CodeforcesLeaderboardResponse>(
            "/api/leaderboard/codeforces"
          );

          if (response.data && response.data.status === "success") {
            // Map the API response to our leaderboard format
            const codeforcesUsers = response.data.data.users.map(
              (user, index) => ({
                rank: index + 1,
                username: user.username,
          ...
          const response = await backend.get<PotdLeaderboardResponse>(
            "/api/leaderboard/potd"
          );
        ...
        <div className="min-w-[80vw] min-h-screen bg-gray-900 w-full">
          <div className="container mx-auto px-4 sm:px-6 lg:px-12 py-8 max-w-full">
            <div className="flex items-center justify-center mb-4 sm:mb-8">
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white font-poltawski">
                Student Leaderboard
              </h1>
            </div>
            {/* Last Updated Date */}
            ...
            <div className="flex justify-center mb-4 sm:mb-8 flex-wrap gap-2">
              {/* make a dropdown here */}
              <div>
                <Dropdown label={year} dismissOnClick={true}>
                  <DropdownItem
                    onClick={() => {
                      setYear("2027");
                    }}
              ...
              <div className="flex flex-wrap justify-center space-x-2 bg-gray-800 rounded-lg p-1">
                ...
                >
                  ...
                  <span className="font-poppins font-xxl text-xs sm:text-sm md:text-base">
                    CodeForces
                  </span>
                </button>
                <button
              ...
              <div className="bg-gray-800 rounded-xl shadow-xl overflow-x-auto">
                <table className="w-full table-auto max-w-full">
                  <thead className="bg-gray-700">
                    <tr>
                      <th className="px-1 sm:px-6 py-4 text-left text-xs font-xxl text-gray-300 uppercase tracking-wider font-poppins">
                        Rank
                      </th>
                      <th className="px-1 sm:px-6 py-4 text-left text-xs font-xxl text-gray-300 uppercase tracking-wider font-poppins">
                        Student
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {(() => {
                      let rank: number = 0;
                      return data[activeTab].map((student) => {
                        console.log(student);
                        // console.log(Number(student.batch));
                        // console.log(Number(year.slice(0, 4)));
                        if (
                          Number(student.batch) == Number(year) ||
                          !student.batch
                        ) {
                          rank++;
                          return (
                            <tr
                              ...
                              className={`hover:bg-gray-700 transition-colors ${getRankColor(
                                student.rank
                              )} ${getRowClass(student)}`}
                            >
                              <td className="px-1 sm:px-6 py-4 whitespace-nowrap">
                                <div className="text-lg font-semibold text-AC_Orange font-poltawski">
                                  {/* #{student.rank} */}#{rank}
                                </div>
                              </td>
                              <td className="px-1 sm:px-6 py-4 whitespace-nowrap">
                                <div className="text-sm font-xxl text-white font-poppins">
                                  ...
                                  {activeTab === "codeforces" &&
                                    student.cfRank && (
                                      <span
                                        className={`ml-2 text-xs px-1 py-0.5 rounded font-semibold ${getCodeforcesRankColor(
                                          student.cfRank
