...
function WinnersTile({data}) {
    return (
        <div
            className="relative group bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-500 transform hover:-translate-y-1 hover:bg-gradient-to-b hover:from-white hover:to-orange-50/30 overflow-hidden">
            {/* Glow effect on hover */}
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <div
                    className="absolute inset-0 bg-gradient-to-br from-AC_Orange/10 via-transparent to-AC_Green/10"></div>
            </div>

            <div className="relative flex flex-col items-center py-3">
                {/* Top achievement bar */}
                <div className="w-full bg-gradient-to-r from-AC_Orange/90 to-AC_Orange mb-3 -mt-3 px-3 py-2 text-center
                    ...
                    {data.names.map((name, index) => (
                        <div key={index}
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-AC_Orange text-xs opacity-75 group-hover:opacity-100">◆</span>
                                <span
                                    className="text-gray-700 text-sm group-hover:text-gray-800 transition-colors duration-300">
                                    {name.value}
                                </span>
                            </div>
                            <a
            ...
            {/* Bottom accent line */}
            <div className="absolute bottom-0 h-0.5 w-full bg-gradient-to-r from-AC_Green/60 via-AC_Green to-AC_Green/60
                group-hover:from-AC_Green/80 group-hover:via-AC_Green group-hover:to-AC_Green/80
                group-hover:h-1 transition-all duration-500"></div>
        </div>
    )
